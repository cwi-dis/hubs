...
import { Matrix4, Mesh, MeshBasicMaterial, MeshStandardMaterial } from "three";
...
export function releaseOccupiedWaypoint(world: HubsWorld) {
  if (myOccupiedWaypoint) {
    NetworkedWaypoint.occupied[myOccupiedWaypoint] = 0;
    console.log("clearing mine");
  ...
  waypointQuery(world).forEach(eid => {
    ...
    if (hasComponent(world, Interacted, eid)) {
      if (hasComponent(world, NetworkedWaypoint, eid)) {
        if (NetworkedWaypoint.occupied[eid]) {
          console.log("Can't travel to occupied waypoint...");
        } else {
          console.log("Taking ownership and traveling...");
